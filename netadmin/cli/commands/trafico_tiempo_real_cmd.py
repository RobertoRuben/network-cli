...
@app.command("monitor", help="Monitorear el tráfico de red por dispositivo en tiempo real")
def comando(
    ip: str = typer.Argument(..., help="Dirección IP base para monitoreo (ej: 192.168.1.1)"),
    duracion: int = typer.Option(60, "--duracion", "-d", help="Duración en segundos del monitoreo"),
    intervalo: int = typer.Option(1, "--intervalo", "-i", help="Intervalo de actualización en segundos"),
    cantidad: int = typer.Option(5, "--cantidad", "-c", help="Cantidad de dispositivos a monitorear")
):
    ...
    try:
        ...
        with Live(generar_layout(generar_tabla()), refresh_per_second=4) as live:
            ...
            for datos in monitor:
                ...
                if datos:
                    for dato in datos:
                        estadisticas['total_enviado'] += dato.get('bytes_enviados_inc', 0)
                        estadisticas['total_recibido'] += dato.get('bytes_recibidos_inc', 0)
                        ...
                        if estadisticas['dispositivo_max_trafico'] is None or trafico_total > estadisticas.get('max_trafico_total', 0):
                            estadisticas['max_trafico_total'] = trafico_total
                            estadisticas['dispositivo_max_trafico'] = f"{dato['ip']} ({dato['nombre']})"
