...
def comando(
    ip: str = typer.Argument(..., help="Dirección IP base para escanear (ej: 192.168.1.1)"),
    solo_activos: bool = typer.Option(False, "--activos", "-a", help="Mostrar solo dispositivos activos"),
    duracion: int = typer.Option(5, "--duracion", "-d", help="Duración del monitoreo de tráfico en segundos")
):
    ...
    tabla = console_manager.crear_tabla(
        f"Dispositivos de Red - {ip.rsplit('.', 1)[0]}.0/24", 
        columnas
    )
    ...
    for dispositivo in dispositivos:
        ...
        estado_formateado = f"[green]Activo[/]" if estado == 'up' else f"[{COLORS['error']}]Inactivo[/]"
...
def activos(
    ip: str = typer.Argument(..., help="Dirección IP base para escanear (ej: 192.168.1.1)"),
    duracion: int = typer.Option(3, "--duracion", "-d", help="Duración del monitoreo de tráfico en segundos")
):
